...
function fakeP5({ width = 200, height = 100 } = {}) {
    ...
    const rec = (name) =>
        (...args) => {
            ...
            } else if (/^(stroke|fill|strokeWeight|noStroke|noFill|textSize)$/.test(name) && depth === 0) {
                styleOutsidePush.push(name);
            }
            calls.push([name, ...args]);
        };
    const p = { width, height, calls, get styleOutsidePush() { return styleOutsidePush; }, get minDepth() { return minDepth; }, get depth() { return depth; } };
    for (const m of [
        "push", "pop", "line", "rect", "point", "text", "fill", "stroke",
        "strokeWeight", "noStroke", "noFill", "textSize", "translate", "rotate",
        "scale", "triangle", "circle",
    ]) {
        p[m] = rec(m);
    }
    return p;
